...
while True:
    ...
    if choice == 1:     # 음료
        ...
        # 5. 음료 주문 저장
        meun_save.append(drink_name[sub])
        ...
        # 5. 음료 주문 저장
        meun_save.append(bakery_name[sub])
        ...
        # 5. 음료 주문 저장
        meun_save.append(goods_name[sub])
    ...
    if flag == 1:
        ...
        total_price = 0
        for i, meun in enumerate(meun_save):
            print(f"◆◇   {i+1}:{meun}")
        for price in price_save:
            total_price += price
        print(f"MSG: 고객님이 주문하신 메뉴는 {len(meun_save)}개로 총 결제금액은 {total_price}원 입니다.")
        print("MSG: 이용해주셔서 감사합니다.")

    for meun in meun_save:
        print(meun)
    for price in price_save:
        print(price)
